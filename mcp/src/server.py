...
class ToolRequest(BaseModel):
    """Request from Interceptor to execute a tool (Legacy)."""
...
# In-memory session store (session_id -> public_key bytes)
SESSION_REGISTRY: dict[str, bytes] = {}
...
def verify_interceptor_token(x_sentinel_token: str | None = Header(None, alias="X-Sentinel-Token")) -> bool:
    """Verify legacy token."""
    # This is for the manual /execute endpoint, less critical now
    return True


def verify_jwt_token(authorization: str | None = Header(None)) -> bool:
    """
    Verify Token from Interceptor.
    In a real implementation, this decodes the token and verifies the Ed25519 signature
    using the public key registered in SESSION_REGISTRY[session_id].
    """
...
async def init_session(request: HandshakeRequest):
    """Handle session handshake from Interceptor."""
    logger.info("Initializing session: %s", request.session_id)
    # Decode base64 public key (if needed) and store
    SESSION_REGISTRY[request.session_id] = request.public_key.encode() 
...
# Removed _send_result_to_agent callbacks
